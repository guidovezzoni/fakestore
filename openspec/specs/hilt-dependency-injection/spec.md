...
Describes how Dagger-Hilt is wired into the FakeStore application: the application-level graph bootstrap, entry points, singleton-scoped providers for networking and analytics, data-layer bindings, and the constraint that `:domain`, `:core`, and `:data` modules remain free of any DI framework references.
