...
This capability defines the domain model, DTO layer, mapping, API service extension, repository contract and implementation, and use case for fetching a user profile from the Fake Store API. It covers the full data pipeline from network response through to a clean domain `UserProfile` object.
